...
def Add_Instructor(request):
    ...
    if request.method == 'POST':
        ...
        g = request.POST['gender']
        p = request.POST['plan']
        members_pt = str(request.POST.getlist('username'))
        plan = Plan.objects.filter(name=p).first()
        try:
            Instructor.objects.create(name=n, contact=c, emailid=e, age=a, gender=g, plan=plan, members=members_pt)
            error = "no"
        except:
            error = "yes"
    d = {'error': error, 'plan':plan1, 'users':members}
    return render(request, 'add_instructor.html', d)
...
def Delete_Instructor(request, pid):
    instructor = Instructor.objects.get(id=pid)
    instructor.delete()
